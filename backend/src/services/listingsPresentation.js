...
function mapListingSearchRow(row) {
  ...
  return {
    ...
    is_troc: toBoolean(row.is_troc),
    troc_wants: Array.isArray(row.troc_wants) ? row.troc_wants : [],
    troc_accepts_complement_xpf: toBoolean(row.troc_accepts_complement_xpf),
    troc_complement_max_xpf: toNumber(row.troc_complement_max_xpf, 0),
    troc_status: row.troc_status ?? 'open',
...
function buildSellerPayload(listing) {
  return {
    ...
    is_online: listing.seller_is_online ?? false,
    last_seen_at: listing.seller_last_seen_at ?? null,
    last_seen_label: listing.seller_last_seen_label ?? null,
    avg_response_time_minutes: listing.seller_avg_response_time_minutes ?? null,
    avg_response_time_label: listing.seller_avg_response_time_label ?? null,
  }
}
...
function mapListingDetailResponse(listing, isFavorited = false) {
  ...
  return {
    data: {
      ...
      troc_wants: Array.isArray(listing.troc_wants) ? listing.troc_wants : [],
      troc_accepts_complement_xpf: Boolean(listing.troc_accepts_complement_xpf),
      troc_complement_max_xpf: listing.troc_complement_max_xpf ?? 0,
      troc_status: listing.troc_status ?? 'open',
      compatibility: listing.compatibility ?? null,
      metadata: listing.metadata && typeof listing.metadata === 'object' ? listing.metadata : {},
