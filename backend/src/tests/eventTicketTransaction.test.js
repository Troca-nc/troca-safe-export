...
async function run() {
  ...
  await check('Ticket validation and finalization share one connection and commit before email', async () => {
    const h = harness();
    await h.invoke();
    ...
    assert.strictEqual(h.trace.at(-1), 'EMAIL');
    assert.match(h.trace[2], /provider = 'stripe'.*FOR UPDATE/);
  });
  for (const failOn of ['UPDATE ticket_types', 'UPDATE tickets ', 'UPDATE ticket_orders', 'UPDATE events', 'UPDATE payments', 'COMMIT']) {
  ...
  await check('Email failure after commit does not pretend to roll back paid tickets', async () => {
