...
const requestSchema = communeLookupSchema.keys({
  service_type: Joi.string().valid('colis', 'demenagement', 'fret_pro').default('fret_pro'),
  cargo_type: Joi.string().trim().min(2).max(200).allow('', null).optional(),
  type_marchandise: Joi.string().trim().min(2).max(200).allow('', null).optional(),
  poids: Joi.string().trim().max(80).allow('', null).optional(),
  fragile: Joi.boolean().optional(),
  volume: Joi.string().trim().max(80).allow('', null).optional(),
  etage_depart: Joi.string().trim().max(80).allow('', null).optional(),
  etage_arrivee: Joi.string().trim().max(80).allow('', null).optional(),
  manutention: Joi.boolean().optional(),
  nb_pieces: Joi.string().trim().max(80).allow('', null).optional(),
...
router.delete('/:id/offers/mine', authenticate, async (req, res, next) => {
  try {
    const requestId = Number(req.params.id);
    if (!Number.isFinite(requestId) || requestId <= 0) {
      return res.status(400).json({ error: 'Demande invalide.' });
