...
> = {
  ...
  pro: {
    title: 'Compte Pro',
    subtitle: 'Gestion du catalogue, statistiques et visibilitï¿½',
  ...
  bon_plan: {
    ...
    subtitle: 'Promo, ï¿½vï¿½nement ou mise en avant locale',
    accent: 'from-lagoon/20 to-[var(--color-surface)]',
    badge: 'Bon plan sponsorisï¿½',
    depositLabel: 'Contenu sponsorisï¿½',
    accountLabel: 'Espace bon plan',
    accountHint: 'Programmer une promo, suivre la visibilitï¿½ et la diffusion.',
  },
}
    ...
    return (
      <div className="rounded-[1.5rem] border border-night/10 bg-white dark:bg-[var(--color-surface)] p-5 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-night">Choisissez un profil pour voir l'aperï¿½u.</p>
            <p className="text-sm text-night/60">
              Choisissez un profil dï¿½mo dans le header pour voir la publication et le compte comme un vrai utilisateur.
            </p>
          </div>
        </div>
      </div>
    ...
    <div className={`rounded-[1.75rem] border border-night/10 bg-gradient-to-br ${config.accent} p-5 shadow-[0_20px_70px_rgba(8,32,50,0.08)]`}>
      <div className="mb-4 flex items-center justify-between gap-3 rounded-2xl border border-white/70 bg-white/75 dark:bg-[var(--color-surface)] px-4 py-3 shadow-sm backdrop-blur">
        <div className="flex items-center gap-3">
          ...
          <div>
                        {/* Profil actif supprimï¿½ pour garder le contexte */}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <span className="rounded-full border border-night/10 bg-white dark:bg-[var(--color-surface)] px-3 py-1 text-xs font-semibold text-night/70">
          {mode === 'deposit' ? 'Comment votre annonce apparaï¿½t' : 'Votre espace personnel'}
        </span>
      </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-2xl border border-white/70 bg-white/90 dark:bg-[var(--color-surface)] p-4">
            ...
            <div className="mt-3 space-y-2">
              ...
              <div className="rounded-xl border border-night/10 bg-sand px-3 py-2 text-sm text-night/70">Prix et nï¿½gociation</div>
          ...
          {activeProfile === 'particulier' ? (
            <div className="mt-4 rounded-2xl border border-night/10 bg-white dark:bg-[var(--color-surface)] p-4 shadow-sm">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-sm font-semibold text-night">Vous n'avez pas encore d'annonce. Publiez la vï¿½tre.</p>
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-3">
              {[
                activeProfile === 'bon_plan'
                  ? { title: 'Campagne ï¿½ crï¿½er', meta: 'Programmez votre premiï¿½re mise en avant.' }
                  : { title: 'Annonce ï¿½ publier', meta: 'Prï¿½parez votre premiï¿½re publication.' },
                activeProfile === 'bon_plan'
                  ? { title: 'Diffusion en attente', meta: 'Le contenu sponsorisï¿½ sera visible ici.' }
                  : { title: 'Message ï¿½ traiter', meta: 'Les ï¿½changes safficheront ici.' },
                activeProfile === 'bon_plan'
                  ? { title: 'Statistiques ï¿½ suivre', meta: 'Suivi de visibilitï¿½ et clics.' }
                  : { title: 'ParamÃ¨tres ï¿½ complï¿½ter', meta: 'Retrouvez vos prï¿½fï¿½rences ici.' },
              ].map((item) => (
