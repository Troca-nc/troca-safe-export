...
export default function SearchAlertModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(response.response?.data?.error ?? 'Impossible de crï¿½er lalerte pour le moment.')
    } finally {
      setSaving(false)
    }
  }
          ...
          {!isSuccess ? (
            <>
              <div className="rounded-[1.5rem] border border-night/8 bg-sand/25 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-night/65">
                  {preview.loading
                    ? 'Calcul des rï¿½sultats en cours...'
                    : preview.error || `${resultCount.toLocaleString('fr-FR')} annonces correspondent actuellement ï¿½ ces critï¿½res.`}
                </p>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]">
                ...
                <div className="rounded-[1.5rem] border border-lagoon/20 bg-lagoon/8 p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-lagoon">Rï¿½sumï¿½</p>
                  <div className="mt-3 space-y-2 text-sm text-night/65">
                    <p>
                      <span className="font-semibold text-night">Rï¿½sultats actuels</span>
                      <br />
                      {preview.loading
                        ? 'Chargement...'
            ...
            <div className="rounded-[1.5rem] border border-emerald-200 bg-emerald-50 p-5">
              <p className="text-sm font-semibold text-emerald-900">
                {resultCount.toLocaleString('fr-FR')} annonces correspondent actuellement ï¿½ ces critï¿½res.
              </p>
              <p className="mt-2 text-sm leading-6 text-emerald-900/75">
                Votre alerte a ï¿½tï¿½ activï¿½e. Un email de confirmation a ï¿½tï¿½ envoyï¿½ avec le rï¿½sumï¿½ des critï¿½res et le lien de gestion.
              </p>
        ...
        {!isSuccess ? (
          <div className="flex flex-col-reverse gap-3 border-t border-night/8 px-5 py-4 sm:flex-row sm:justify-end md:px-6">
            ...
            >
              ...
              {saving ? 'Crï¿½ation&' : 'Activer lalerte'}
            </button>
          </div>
        ) : null}
      </div>
