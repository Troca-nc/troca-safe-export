...
function replyHint(category: ContactCategory) {
  switch (category) {
    ...
    default:
      return 'Reponse sous 24 a 48 heures ouvrées en moyenne.'
  }
}
    ...
    <section className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-2xl">
          ...
          <p className="mt-2 text-sm leading-relaxed text-night/60">
            Remplissez le formulaire ci-dessous pour une réponse structurée. Les comptes connectés ont leurs informations pré-remplies.
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4">
        ...
        <label className="space-y-2">
          ...
          <textarea
            ...
            placeholder="Décrivez votre demande avec un maximum de détails utiles."
            required
          />
        </label>
        ...
        {success ? (
          <FeedbackAlert tone="success" title="Message envoyé">
            {success}
          </FeedbackAlert>
        ) : null}
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background-secondary)] p-4">
          ...
          <p className="mt-2 text-sm font-semibold text-night">24h à 48h ouvrées</p>
          <p className="mt-1 text-xs text-night/55">Temps de réponse moyen sur les demandes standard.</p>
        </div>
      </div>
    </section>
  )
