...
export default function ProQuoteModal({ proId, proName, open, onClose, template, prefill, onSent }: ProQuoteModalProps) {
  ...
  const budgetLabel = useMemo(() => {
    ...
    if (!Number.isFinite(amount) || amount <= 0) return 'Budget ï¿½ dï¿½finir'
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showToast({
        tone: 'success',
        title: 'Demande de devis envoyï¿½e',
        message: `${proName} a reï¿½u votre demande et peut vous rï¿½pondre rapidement.`,
      })
    } catch (err: any) {
        ...
        <div className="flex items-start justify-between gap-4 border-b border-[var(--color-border)] px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm text-night/55">
              {quoteTemplate.subtitle} Le professionnel {proName} reï¿½oit la demande immï¿½diatement.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-0 lg:grid-cols-[1fr_0.9fr]">
            ...
            {sent ? (
              <div className="flex h-full flex-col justify-center">
                <FeedbackAlert tone="success" title="Demande envoyï¿½e !">
                  {proName} a reï¿½u votre demande de devis. Vous pouvez suivre vos ï¿½changes depuis votre messagerie.
                </FeedbackAlert>
          ...
          <aside className="space-y-4 px-6 py-6">
            <div className="rounded-[1.5rem] border border-[var(--color-border)] bg-[var(--color-background-secondary)] p-4">
              ...
              <div className="mt-3 space-y-3 text-sm text-night/65">
                <p className="flex items-start gap-2">
                  ...
                  <span>Votre demande part directement au professionnel sï¿½lectionnï¿½.</span>
                </p>
                <p className="flex items-start gap-2">
                  <MapPin className="mt-0.5 h-4 w-4 text-[#0A7EA4]" />
                  <span>Vous pouvez prï¿½ciser la commune pour un retour plus rapide.</span>
                </p>
                {quoteTemplate.show_budget ? (
                  <p className="flex items-start gap-2">
                    <CreditCard className="mt-0.5 h-4 w-4 text-[#0A7EA4]" />
                    <span>Budget estimï¿½ actuel : {budgetLabel}.</span>
                  </p>
                ) : null}
                {quoteTemplate.show_date ? (
                  <p className="flex items-start gap-2">
                    <CalendarDays className="mt-0.5 h-4 w-4 text-[#0A7EA4]" />
                    <span>La date souhaitï¿½e permet au pro de prioriser votre demande.</span>
                  </p>
                ) : null}
              </div>
            </div>
            ...
            <div className="rounded-[1.5rem] border border-emerald-100 bg-emerald-50/70 p-4 text-sm text-night/70">
              ...
              <p className="mt-2">
                Plus votre besoin est clair, plus le professionnel peut rï¿½pondre vite et avec une estimation
                rï¿½aliste.
              </p>
            </div>
          </aside>
