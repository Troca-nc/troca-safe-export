...
const BOOST_PLANS: BoostPlan[] = [
  { days: 3, price: 990, label: 'Idï¿½al pour tester' },
  { days: 7, price: 1990, label: 'Le plus populaire', highlight: true },
  { days: 30, price: 5900, label: 'Visibilitï¿½ maximale' },
]
      ...
      <div className="w-full max-w-2xl rounded-3xl bg-[var(--color-surface)] p-6 shadow-2xl transition-all duration-200 animate-scale-in">
        {success ? (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            <h3 className="mt-4 font-display text-2xl font-bold text-night">Boost activï¿½ !</h3>
          ...
          <>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-1 font-display text-2xl font-bold text-night">ï¿½ Booster cette annonce</h3>
                <p className="mt-1 text-sm text-night/60">Votre annonce apparaï¿½t en tï¿½te des rï¿½sultats et sur la homepage.</p>
              </div>
              <button
                type="button"
                onClick={onClose}
            ...
            <div className="mt-5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-background-secondary)] p-4">
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                <span className="text-sm text-night/60">
                  {selectedPlan.days} jours ï¿½ {selectedPlan.price.toLocaleString('fr-FR')} XPF
                </span>
                <button
                  type="button"
                  onClick={handlePay}
