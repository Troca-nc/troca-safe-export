...
export default function BookingButton({
  ...
  const submitBooking = async (payloadMessage?: string) => {
    ...
    try {
      ...
      if (status === 'pending') {
        setSuccess({ mode: 'manual', label: 'ï¿½ Demande envoyï¿½e - rï¿½ponse sous 24h' })
      } else {
        setSuccess({ mode: 'auto', label: ' Place rï¿½servï¿½e !' })
      }
      ...
      setError(err?.response?.data?.error || err?.message || 'Impossible de rï¿½server ce trajet.')
    } finally {
      setLoading(false)
    }
  }
              ...
              <form onSubmit={handleManualSubmit} className="space-y-4">
                <label className="block">
                  ...
                  <textarea
                    ...
                    rows={5}
                    placeholder="Bonjour, je suis intï¿½ressï¿½(e) par cette place..."
