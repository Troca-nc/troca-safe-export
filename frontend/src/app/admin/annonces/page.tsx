...
import type { AdminAnnonce, StatutAnnonce } from '@/types/admin.types'
import { format, parseISO } from 'date-fns'
import { fr } from 'date-fns/locale'
import Link from 'next/link'

type AnnoncesFilters = {
  search?: string
  statut?: StatutAnnonce | 'toutes'
  categorie?: string
  page: number
  limit: number
  sort: 'created_at' | 'nb_vues' | 'nb_signalements'
  order: 'asc' | 'desc'
}
...
const STATUT_STYLE: Record<StatutAnnonce, string> = {
  ...
  'signalï¿½e':  'bg-red-50 text-red-600',
  suspendue: 'bg-red-50 text-red-500',
  'expirï¿½e':   'bg-sand text-night/40',
...
const FILTRES_STATUT: { label: string; value: StatutAnnonce | 'toutes' }[] = [
  ...
  { label: 'Signalï¿½es', value: 'signalï¿½e' },
  { label: 'Suspendues',value: 'suspendue' },
  { label: 'Expirï¿½es',  value: 'expirï¿½e' },
]
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              placeholder="Titre, catï¿½gorie, utilisateur&"
        ...
        <div className="bg-white border border-night/8 rounded-2xl overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-night/8 bg-sand/50">
                ...
                <th className="px-4 py-3 text-left text-[10px] font-medium text-night/50 uppercase tracking-wide">CatÃ©gorie</th>
          ...
          {!loading && (data?.data.length ?? 0) === 0 && (
            <div className="text-center py-12 text-night/40">
              <p className="text-sm">Aucune annonce trouvï¿½e</p>
            </div>
          )}
        </div>
