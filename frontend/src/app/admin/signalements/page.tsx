...
//  Gestion des signalements avec moderation en un clic 
      ...
      <div className="flex items-center justify-between px-4 py-3">
        <div className="flex items-center gap-3">
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${URGENCE_STYLE[s.urgence]}`}>
            {s.urgence === 'haute' ? 'e Urgent' : s.urgence === 'moyenne' ? 'Modere' : 'Faible'}
          </span>
          <div>
            ...
            <p className="text-xs text-night/50">
              ...
              {' ï¿½ '}par <span className="font-medium">{s.user_signale.nom}</span>
            </p>
          </div>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-night/8 pt-3 bg-sand/30">
          ...
          <input
            ...
            placeholder="Raison de la decision (optionnel)"
      ...
      <div className="p-6 overflow-y-auto flex-1">
        ...
        {(data?.total ?? 0) > (data?.data.length ?? 0) && (
          <p className="text-center text-xs text-night/40 mt-4 flex items-center justify-center gap-1">
            <Clock size={12} />
            {data?.total} signalements au total - affichage des plus recents
          </p>
        )}
      </div>
    </AdminLayout>
