...
export default function DemoQaPage() {
  ...
  const demoBadge = useMemo(() => demoEmail.endsWith('@demo.kalico') ? 'Compte d�mo actif' : 'Mode r�el', [demoEmail])
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      setMessage(`Jeu de donn�es g�n�r�: ${result?.counts?.users ?? 0} comptes, ${result?.counts?.listings ?? 0} annonces.`)
      await loadStatus()
    } finally {
      setBusy(null)
    }
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setMessage(result?.cleared ? 'Jeu de donn�es d�mo supprim�.' : 'Jeu de donn�es remis � z�ro.')
      await loadStatus()
    } finally {
      setBusy(null)
    }
  ...
  const loginAs = async (key: keyof typeof DEMO_ACCOUNTS) => {
    ...
    try {
      ...
      setMessage(err?.response?.data?.error ?? 'Connexion d�mo impossible.')
    } finally {
      setBusy(null)
    }
  }
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl">
                ...
                <p className="mt-3 max-w-2xl text-sm leading-relaxed text-white/72 md:text-base">
                  G�n�rez les donn�es locales, ouvrez les r�les instantan�s, puis naviguez dans toutes les pages critiques
                  comme un utilisateur r�el, sur web et mobile.
                </p>
              </div>
              <div className="grid gap-3 sm:grid-cols-3 lg:min-w-[24rem]">
                <div className="rounded-2xl border border-white/10 bg-white/8 p-4">
                  ...
                  <p className="mt-2 text-sm font-semibold">{status?.enabled ? 'Local activ�' : 'Mode hors ligne'}</p>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/8 p-4">
                  ...
                  <p className="text-xs text-white/60">comptes d�mo</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/8 p-4">
                  ...
                  <p className="text-xs text-white/60">annonces seed�es</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-6">
              <div className="rounded-[1.75rem] border border-night/8 bg-white p-6 shadow-card">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <h2 className="text-xl font-bold text-night">Cr�er ou vider les donn�es de d�monstration</h2>
                  </div>
                </div>
                ...
                <div className="mt-4 rounded-2xl bg-sand px-4 py-3 text-sm text-night/65">
                  Les comptes d�mo utilisent le mot de passe commun <span className="font-semibold">Demo1234!</span>.
                  Le seed cr�e des annonces, messages, notifications, avis, paiements simul�s et �v�nements analytics.
                </div>
              </div>
              ...
              <div className="rounded-[1.75rem] border border-night/8 bg-white p-6 shadow-card">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-ocean/80">Connexion instantan�e</p>
                    <h2 className="text-xl font-bold text-night">Acc�der aux r�les r�els seed�s</h2>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-[1.75rem] border border-night/8 bg-white p-6 shadow-card">
                ...
                <div className="mt-4 rounded-2xl border border-dashed border-night/10 bg-night/[0.03] p-4 text-sm text-night/60">
                  Sur mobile, installez le seed puis utilisez les boutons de connexion instantan�e dans l�cran de login
