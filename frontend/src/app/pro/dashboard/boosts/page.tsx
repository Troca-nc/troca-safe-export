      ...
      <section className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-night/60">Suivez vos boosts actifs et vos anciennes campagnes de visibilité.</p>
          </div>
      ...
      <section className="space-y-4">
        ...
        {activeBoosts.length > 0 ? (
          ...
          <div className="rounded-[1.75rem] border border-dashed border-[var(--color-border)] bg-[var(--color-surface)] px-6 py-12 text-center text-night/55">
            ...
            <p className="mt-2 text-sm">Boostez une annonce pour la faire remonter sur la homepage et dans sa catégorie.</p>
          </div>
        )}
      </section>
                  ...
                  <tr key={boost.id}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${
                        ...
                        {boost.status === 'active' ? 'Actif' : boost.status === 'expired' ? 'Expiré' : 'Annulé'}
                      </span>
                    </td>
                  </tr>
                ))}
