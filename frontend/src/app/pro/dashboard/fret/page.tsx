...
const SLOT_LABELS = {
  morning: 'Matin',
  midday: 'Midi',
  afternoon: 'Aprï¿½s-midi',
  evening: 'Fin de journï¿½e',
}
...
export default function ProDashboardFretPage() {
  ...
  const handleSubmitOffer = async (requestId: number) => {
    ...
    try {
      ...
      setSuccess('Votre offre a bien ï¿½tï¿½ envoyï¿½e.')
      await load()
    } catch (err: any) {
  ...
  const handleMarkDelivered = async (requestId: number) => {
    ...
    try {
      ...
      setSuccess('Transport marquï¿½ comme livrï¿½.')
      await load()
    } catch (err: any) {
      setError(err?.response?.data?.error || 'Impossible de marquer ce transport comme livrï¿½.')
    } finally {
      ...
      <section className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 font-display text-3xl font-bold text-night">Demandes en cours et offres privï¿½es</h1>
            <p className="mt-2 text-sm text-night/60">
              Rï¿½pondez aux demandes ouvertes, suivez vos offres et marquez vos transports comme livrï¿½s depuis le dashboard Pro.
          </p>
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs text-night/60">
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-sand px-2.5 py-1">
            ...
            {transporter?.is_verified ? 'Pro Envoi & Livraison vï¿½rifiï¿½' : 'Profil ï¿½ vï¿½rifier'}
          </span>
          <span className="inline-flex items-center gap-1 rounded-full bg-sand px-2.5 py-1">
      ...
      <section className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 font-display text-2xl font-bold text-night">Rï¿½pondre rapidement</h2>
            </div>
              ...
              return (
                <article key={request.id} className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-night">{request.departure_commune?.name || request.departure} ï¿½ {request.destination_commune?.name || request.destination}</p>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    <label className="space-y-1">
                      <span className="text-xs font-semibold uppercase tracking-[0.16em] text-night/55">Prix proposï¿½</span>
                      <input
                        type="number"
                        min="1"
                        step="100"
                    ...
                    <label className="space-y-1 sm:col-span-2">
                      <span className="text-xs font-semibold uppercase tracking-[0.16em] text-night/55">Crï¿½neau</span>
                      <select
                  ...
                  {myOffer ? (
                    <p className="mt-3 text-sm text-night/60">
                      Votre offre actuelle : {formatMoney(myOffer.amount_xpf)} ï¿½ {formatDateLabel(myOffer.pickup_date)} ï¿½ {myOffer.status_label}
                    </p>
                  ) : null}
        ...
        <div className="space-y-4">
          <article className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm">
            ...
            <h2 className="mt-1 font-display text-2xl font-bold text-night">Historique des rï¿½ponses</h2>
            <div className="mt-4 space-y-3">
              {myOffers.length ? myOffers.map((offer) => (
                <div key={offer.id} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background-secondary)] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-night">{offer.request_departure || 'Dï¿½part'} ï¿½ {offer.request_destination || 'Arrivï¿½e'}</p>
                      <p className="mt-1 text-xs text-night/55">{formatDateLabel(offer.pickup_date)} ï¿½ {offer.pickup_slot_label}</p>
                    </div>
                ...
                <p className="rounded-2xl border border-dashed border-[var(--color-border)] bg-sand/40 p-4 text-sm text-night/55">
                  Aucune offre envoyï¿½e pour le moment.
                </p>
              )}
            </div>
          </article>
              ...
              {confirmedTransports.length ? confirmedTransports.map((request) => (
                <div key={request.id} className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-background-secondary)] p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-night">{request.departure_commune?.name || request.departure} ï¿½ {request.destination_commune?.name || request.destination}</p>
                      <p className="mt-1 text-xs text-night/55">{formatDateLabel(request.selected_offer?.pickup_date)} ï¿½ {request.selected_offer?.pickup_slot}</p>
                    </div>
                ...
                <p className="rounded-2xl border border-dashed border-[var(--color-border)] bg-sand/40 p-4 text-sm text-night/55">
                  Aucun transport confirmï¿½ pour le moment.
                </p>
              )}
            </div>
          </article>
