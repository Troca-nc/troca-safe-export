...
const VARIABLE_CHIPS = ['{prï¿½nom_client}', '{nom_entreprise}', '{heure_retour}']
const TIME_OPTIONS = ['00:00', '06:00', '08:00', '09:00', '10:00', '12:00', '13:00', '14:00', '15:00', '17:00', '18:00', '20:00', '22:00']
const DELAY_OPTIONS = [
  { value: 0, label: 'Rï¿½pondre immï¿½diatement' },
  { value: 5, label: 'Aprï¿½s 5 min' },
  { value: 15, label: 'Aprï¿½s 15 min' },
  { value: 30, label: 'Aprï¿½s 30 min' },
] as const
...
function replacePreviewVariables(message: string) {
  return message
    .replaceAll('{prï¿½nom_client}', 'Emma')
...
export default function AutoReplyPage() {
  ...
  const [message, setMessage] = useState(
    'Bonjour, merci pour votre message !\nJe suis actuellement indisponible mais je reviendrai vers vous dï¿½s que possible.\nCordialement, [Votre prï¿½nom]'
  )
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'Impossible de sauvegarder la rï¿½ponse automatique.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-nc-emeraude">Rï¿½ponse automatique</p>
            <h1 className="mt-2 font-display text-3xl font-bold text-night">Rï¿½pondez mï¿½me quand vous ï¿½tes occupï¿½</h1>
            <p className="mt-2 max-w-3xl text-sm leading-relaxed text-night/60">
              Configurez un message automatique pour rassurer vos clients et garder le contact, mï¿½me en dehors de vos horaires.
            </p>
          </div>
          ...
          >
            ...
            {isActive ? 'Rï¿½ponse automatique activï¿½e' : 'Rï¿½ponse automatique dï¿½sactivï¿½e'}
          </button>
        </div>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
        <div className={`rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm ${isActive ? '' : 'opacity-80'}`}>
          ...
          <p className="mt-2 text-sm text-night/55">Variables disponibles: {VARIABLE_CHIPS.join(' ï¿½ ')}</p>
          ...
          <div className="mt-2 flex items-center justify-between text-xs text-night/45">
            <span>{message.length}/500 caractï¿½res</span>
            <span className="inline-flex items-center gap-1">
              <Sparkles className="h-3.5 w-3.5" />
              Aperï¿½u en temps rï¿½el
            </span>
          </div>

          <div className="mt-6 rounded-[1.5rem] border border-[var(--color-border)] bg-[var(--color-background-secondary)] p-4">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-nc-lagon">Aperï¿½u du message</p>
        ...
        <div className="space-y-6">
                  ...
                  <div key={day.value} className="grid gap-3 rounded-2xl border border-[var(--color-border)] p-3 md:grid-cols-[140px_1fr] md:items-center">
                    ...
                    {enabled ? (
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-sm text-night/45">ï¿½</span>
                        <select
                          value={activeUntil}
          ...
          <section className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
            <h2 className="font-display text-2xl font-bold text-night">Dï¿½lai de rï¿½ponse</h2>
            <p className="mt-2 text-sm text-night/55">Choisissez quand la rï¿½ponse partira automatiquement.</p>
            <select
              value={delayMinutes}
            ...
            {saved ? <p className="mt-3 text-sm font-semibold text-emerald-700"> Rï¿½ponse automatique configurï¿½e</p> : null}
