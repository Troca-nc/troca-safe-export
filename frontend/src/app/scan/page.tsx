...
export default function ScanPage() {
  ...
  const [status, setStatus] = useState('En attente de la camï¿½ra...')
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        setError(err?.message || 'Impossible daccï¿½der ï¿½ la camï¿½ra.')
      }
    }
      ...
      <section className="mx-auto max-w-4xl px-4 py-8">
        <div className="rounded-[2rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-6 shadow-sm">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-night/65">
            Pointez la camï¿½ra vers le QR code gï¿½nï¿½rï¿½ par la billetterie native pour ouvrir la fiche du billet.
          </p>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.8fr]">
            ...
            <aside className="space-y-4 rounded-[1.5rem] border border-[var(--color-border)] bg-[var(--color-background-secondary)] p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-night">Ãtat</p>
              ...
              <div className="rounded-2xl border border-white/70 bg-white p-4">
                ...
                <ul className="mt-2 space-y-2 text-sm text-night/65">
                  ...
                  <li className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" /> La lumiï¿½re doit ï¿½tre suffisante.</li>
                  <li className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" /> Le QR peut aussi ï¿½tre saisi manuellement.</li>
                </ul>
              </div>
