...
export default function MesRdvPage() {
  ...
  const handleAction = async (bookingId: number | string, action: 'confirm' | 'decline' | 'cancel') => {
    ...
    try {
      ...
      showToast({
        tone: 'success',
        title: 'Rendez-vous mis ï¿½ jour',
        message: 'La rï¿½servation a ï¿½tï¿½ actualisï¿½e.',
      })
    } catch (err: any) {
      const message = err?.response?.data?.error || 'Impossible de mettre ï¿½ jour ce rendez-vous.'
      setError(message)
      showToast({
        tone: 'error',
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 md:py-12">
        <section className="rounded-[2rem] border border-night/8 border-b-4 border-b-nc-corail bg-[linear-gradient(135deg,_rgba(8,32,50,0.98),_rgba(10,126,164,0.18))] px-6 py-8 text-white shadow-[0_24px_80px_rgba(8,32,50,0.12)]">
          ...
          <h1 className="mt-4 font-display text-4xl font-bold md:text-5xl">
            Regroupez vos demandes, vos rï¿½ponses et vos crï¿½neaux au mï¿½me endroit.
          </h1>
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-white/70 md:text-base">
            Suivez les rendez-vous que vous avez demandï¿½s et les demandes que vous avez reï¿½ues, avec un accï¿½s direct ï¿½ la messagerie pour se coordonner rapidement.
          </p>
        </section>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-4">
          ...
          <article className="rounded-[1.75rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm">
            <p className="text-sm font-semibold text-night/55">ï¿½ venir</p>
            <p className="mt-2 text-3xl font-bold text-night">{counts.upcoming.toLocaleString('fr-FR')}</p>
          </article>
          <article className="rounded-[1.75rem] border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm">
            <p className="text-sm font-semibold text-night/55">Demandes reï¿½ues</p>
          ...
          {loading ? (
            ...
            <div className="rounded-[1.75rem] border border-dashed border-night/10 bg-white p-8 text-center text-night/55">
              ...
              <p className="mt-4 text-lg font-semibold text-night">
                {tab === 'client' ? 'Aucune demande en cours' : tab === 'pro' ? 'Aucune demande reï¿½ue' : 'Aucun rendez-vous'}
              </p>
              <p className="mt-2 text-sm">
                {tab === 'client'
                  ? 'Vos demandes de rendez-vous apparaï¿½tront ici aprï¿½s envoi.'
                  : tab === 'pro'
                    ? 'Les demandes reï¿½ues depuis votre vitrine Pro apparaï¿½tront ici.'
                    : 'Rï¿½servez un rendez-vous depuis une vitrine Pro pour le voir apparaï¿½tre ici.'}
              </p>
